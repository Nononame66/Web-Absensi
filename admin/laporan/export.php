...
$subtitle = "Periode: " . ($start_date ? date('d/m/Y', strtotime($start_date)) : '-') . " - " . ($end_date ? date('d/m/Y', strtotime($end_date)) : '-');
...
if ($format === 'excel') {
    ...
    $spreadsheet->getProperties()
        ->setCreator('SMKN 1 Sanden Absensi System')
        ->setLastModifiedBy('SMKN 1 Sanden Admin')
        ->setTitle($title)
        ->setSubject('Attendance Report')
        ->setDescription('Attendance report generated by SMKN 1 Sanden Absensi System');
    ...
    $sheet->setCellValue('A2', 'SMKN 1 SANDEN');
    ...
    foreach ($data as $key => $record) {
        ...
        $sheet->setCellValue('B' . $row, $record['tanggal'] ? date('d-m-Y', strtotime($record['tanggal'])) : '-');
        ...
        $sheet->setCellValue('F' . $row, ($record['jam_masuk'] && $record['jam_masuk'] !== '00:00:00') ? date('H:i', strtotime($record['jam_masuk'])) : '-');
    ...
    // Add footer
    $row += 2;
    $sheet->setCellValue('A' . $row, 'Laporan ini digenerate secara otomatis dari Sistem Absensi SMKN 1 Sanden');
    $sheet->mergeCells('A' . $row . ':G' . $row);
    ...
    <body>
        <div class="header">
            <div class="header-content">
                <img class="logo" src="' . $logoData . '" alt="SMKN 1 Sanden">
        ...
        foreach ($data as $row) {
            ...
            $html .= '
                <tr>
                    ...
                    <td class="text-center">' . (($row['jam_masuk'] && $row['jam_masuk'] !== '00:00:00') ? date('H:i', strtotime($row['jam_masuk'])) : '-') . '</td>
