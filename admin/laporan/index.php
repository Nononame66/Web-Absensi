...
foreach ($daily_data as $row) {
    ...
    $date = $row['date'] ? date('d M', strtotime($row['date'])) : date('d M');
    $status = $row['status'];
    $count = $row['count'];
...
<head>
    ...
    <title>Laporan Absensi - SMKN 1 Sanden</title>    
                    ...
                    <form method="GET" id="filterForm" class="space-y-6">
                        <div class="grid grid-cols-1 gap-4">
                            ...
                            <div class="bg-gray-800/30 rounded-lg p-4">
                                <h4 class="text-sm font-medium mb-3 text-teal-300">
                        ...
                        <div class="flex flex-col-reverse sm:flex-row items-center justify-between gap-4 pt-4 border-t border-gray-700">
                            ...
                            <button type="submit" class="w-full sm:w-auto px-5 py-2 bg-teal-500 hover:bg-teal-600 text-white rounded-lg text-sm transition-colors">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                    ...
                    <div class="glass-effect rounded-xl p-4 sm:p-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex justify-between items-center p-3 bg-teal-500/10 border border-teal-500/30 rounded-lg">
                                <div class="flex items-center">
                ...
                <div class="glass-effect rounded-xl overflow-hidden">
                    <div class="p-4 sm:p-6 border-b border-gray-800">
                        ...
                        <p class="text-sm text-gray-400 mt-1">
                            Periode: <?= $start_date ? date('d F Y', strtotime($start_date)) : '-' ?> - <?= $end_date ? date('d F Y', strtotime($end_date)) : '-' ?>
                        </p>
                    </div>
                                ...
                                <tbody class="divide-y divide-gray-800">
                                    <?php foreach ($detail_records as $data): ?>
                                        <tr class="hover:bg-purple-900/5 transition-colors">
                                            ...
                                            <td class="px-6 py-4 text-sm">
                                                <?= ($data['jam_masuk'] && $data['jam_masuk'] !== '00:00:00') ? date('H:i', strtotime($data['jam_masuk'])) : '-' ?>
                                            </td>
                                            <td class="px-6 py-4">
                    ...
                    <?php else: ?>
                        <div class="p-10 text-center">
                            ...
                            <button onclick="resetFilters()" class="mt-4 px-4 py-2 bg-teal-500 hover:bg-teal-600 rounded-lg text-sm transition-colors">
                                <i class="fas fa-redo mr-2"></i> Reset Filter
                            </button>
                        </div>
                    <?php endif; ?>
