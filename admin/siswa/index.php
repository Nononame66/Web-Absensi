...
function getSortIcon($column, $sort_column, $sort_order)
{
    ...
    return $sort_order === 'ASC'
        ? '<i class="fas fa-sort-up text-teal-400"></i>'
        : '<i class="fas fa-sort-down text-teal-400"></i>';
}
...
if (isset($_GET['delete']) && $_GET['delete'] == 'success') {
    $msg = $_GET['msg'] ?? 'Data siswa berhasil dihapus';
    $notification = [
        'type' => 'success',
        'message' => $msg
    ];
...
<head>
    ...
    <title>Data Siswa - SMKN 1 Sanden</title>    
...
<body class="min-h-screen text-white bg-fixed">
            ...
            <div class="max-w-7xl mx-auto">
                ...
                <header class="flex flex-wrap justify-between items-center mb-6 gap-4">
                    <div>
                        ...
                        <p class="text-gray-400 text-sm md:text-base">Kelola data siswa SMKN 1 Sanden</p>
                    </div>
                    <div class="hidden sm:flex gap-2">
                        <button onclick="showDeleteClassModal()" class="px-4 py-2 bg-red-500/20 hover:bg-red-500 text-red-500 hover:text-white border border-red-500/50 hover:border-red-500 rounded-lg flex items-center gap-2 text-sm font-medium transition-colors">
                            <i class="fas fa-trash-alt"></i> Hapus Kelas
                        </button>
                        <a href="import.php" class="px-4 py-2 bg-purple-500 hover:bg-purple-600 rounded-lg flex items-center gap-2 text-sm font-medium transition-colors">
                            <i class="fas fa-file-excel"></i> Import Excel
                        </a>
                        <a href="add.php" class="px-4 py-2 bg-teal-500 hover:bg-teal-600 rounded-lg flex items-center gap-2 text-sm font-medium transition-colors">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 mb-6">
                    ...
                    <div class="glass-effect rounded-lg p-4 flex items-center">
                        <div class="h-10 w-10 rounded-lg bg-teal-500/10 flex items-center justify-center mr-3">
                            <i class="fas fa-users text-teal-400"></i>
                        </div>
                        <div>
                ...
                <div class="glass-effect rounded-xl p-4 md:p-6 mb-6">
                    <div class="flex flex-wrap justify-between items-center mb-4">
                        ...
                        <?php if (!empty($_GET) && isset($_GET['search']) || isset($_GET['kelas']) || isset($_GET['jurusan'])): ?>
                            <a href="index.php" class="text-sm flex items-center gap-1 text-teal-300 hover:text-purple-300">
                    ...
                    <form method="GET" id="filterForm">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-4">
                            <div>
                                ...
                                <div class="relative">
                                    <select name="kelas" class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-teal-400">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <select name="jurusan" class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-teal-400">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="text" name="search" value="<?= htmlspecialchars($search) ?>"
                                        ...
                                        class="w-full bg-gray-800 border border-gray-700 rounded-lg pl-9 pr-9 py-2.5 text-sm text-white focus:outline-none focus:border-teal-400">
                        ...
                        <div class="mt-6 flex justify-end">
                            <button type="submit" class="px-5 py-2.5 bg-teal-500 hover:bg-teal-600 text-white rounded-lg transition-colors text-sm">
                ...
                <div class="glass-effect rounded-xl overflow-hidden">
                        ...
                        <div class="p-4 border-t border-gray-800 flex flex-col sm:flex-row justify-between items-center gap-4">
                            ...
                            <div class="flex space-x-1 order-1 sm:order-2 pagination-compact">
                                ...
                                for ($i = $start_page; $i <= $end_page; $i++) {
                                    $is_current = $i == $page;
                                    $class = $is_current
                                        ? 'px-2 sm:px-3 py-1.5 bg-teal-500 rounded text-white text-sm flex items-center justify-center min-w-[32px] current-page page-number'
                    ...
                    <?php else: ?>
                        <div class="p-10 text-center">
                            ...
                            <?php if (!empty($_GET)): ?>
                                <a href="index.php" class="mt-4 inline-block text-teal-300 hover:text-teal-400">
                ...
                <div class="fixed bottom-4 right-4 lg:hidden">
                    <a href="add.php" class="flex items-center justify-center w-14 h-14 bg-teal-500 hover:bg-teal-600 rounded-full shadow-lg transition-colors">
    ...
    <div id="deleteModal" class="fixed inset-0 flex items-center justify-center z-50 hidden">
        ...
        <div class="glass-effect rounded-lg p-6 w-11/12 max-w-md relative z-10 animate-fade-in">
            ...
            <div class="flex justify-end gap-4">
                <button onclick="hideDeleteModal()" class="px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded-lg text-sm transition-colors">
                    Batal
                </button>
                <form id="deleteForm" method="POST" action="delete.php">
                    <input type="hidden" id="deleteId" name="id" value="">
                    <button type="submit" class="px-4 py-2 bg-red-600 hover:bg-red-700 rounded-lg text-sm transition-colors">
                        Hapus
                    </button>
                </form>
            </div>
    ...
    <!-- Delete Class Modal -->
    <div id="deleteClassModal" class="fixed inset-0 flex items-center justify-center z-50 hidden">
        <div class="fixed inset-0 bg-black bg-opacity-50" onclick="hideDeleteClassModal()"></div>
        <div class="glass-effect rounded-lg p-6 w-11/12 max-w-md relative z-10 animate-fade-in">
            <h3 class="text-xl font-semibold mb-4 text-red-500"><i class="fas fa-exclamation-triangle mr-2"></i>Hapus Satu Kelas</h3>
            <p class="text-gray-300 mb-4 text-sm">Pilih kelas dan jurusan yang ingin dihapus. <strong>Semua data siswa beserta data absensinya</strong> dalam kelas tersebut akan dihapus permanen.</p>
            
            <form method="POST" action="delete_class.php">
                <div class="space-y-4 mb-6">
                    <div>
                        <label class="block text-sm text-gray-400 mb-1">Pilih Kelas</label>
                        <select name="kelas" required class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-red-400">
                    ...
                    <div>
                        <label class="block text-sm text-gray-400 mb-1">Pilih Jurusan</label>
                        <select name="jurusan" required class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-red-400">
                ...
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="hideDeleteClassModal()" class="px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded-lg text-sm font-medium transition-colors">
                        Batal
                    </button>
                    <button type="submit" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-sm font-medium transition-colors">
                        <i class="fas fa-trash-alt mr-1"></i> Ya, Hapus Kelas
                    </button>
                </div>
            </form>
        </div>
    ...
    <script>
        ...
        document.querySelectorAll('#filterForm select[name]').forEach(element => {
        ...
        document.addEventListener('keydown', function(e) {
            if (e.key === 'Escape') {
                ...
                if (!document.getElementById('deleteClassModal').classList.contains('hidden')) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (window.innerWidth < 768) {
                const selectFilters = document.querySelectorAll('#filterForm select[name="kelas"], #filterForm select[name="jurusan"]');
